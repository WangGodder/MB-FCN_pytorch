...
parser.add_argument('--annoPath', default="F:\\datasets\\widerface\\train_winder.list", help='Location of wider face')
...
def train():
    ...
    print('Training MB-FCN on', dataset.name)
    step_index = 0
    if args.visdom:
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        with torch.no_grad():
            if args.cuda:
                ...
                targets = [torch.Tensor(anno.cuda()) for anno in targets]
            else:
                images = torch.Tensor(images)
                targets = [torch.Tensor(anno) for anno in targets]
        # forward
        t1 = time.time()
        out = net(images)
        # backprop
        optimizer.zero_grad()
        loss_l, loss_c = criterion(tuple(out), targets)
        loss = loss_l + loss_c
        loss.backward()
        optimizer.step()
        t2 = time.time()
        ...
        if iteration % 5000 == 0:
            ...
            torch.save(net.state_dict(), args.save_folder + 'Res50_mb_fcn(345)(45)_' +
                       repr(iteration) + '.pth')
    torch.save(net.state_dict(), args.save_folder + 'Res50_mb_fcn(345)(45)' + '.pth')
