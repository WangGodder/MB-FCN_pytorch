...
print('Loading model..')
ssd_net = build_model('test', 640, 2)
net = ssd_net
net.load_state_dict(torch.load('./weights/Res50_mb_fcn(345)(45).pth'))
net.cuda()
net.eval()
...
if __name__ == '__main__':
    ...
    if subset is 'val':
        wider_face = sio.loadmat(
            '/home/guoqiushan/share/workspace/caffe-ssd-s3fd/sfd_test_code/WIDER_FACE/wider_face_val.mat')  # Val set
    else:
        wider_face = sio.loadmat(
            '/home/guoqiushan/share/workspace/caffe-ssd-s3fd/sfd_test_code/WIDER_FACE/wider_face_test.mat')  # Test set
    ...
    for index, event in enumerate(event_list):
        ...
        for num, file in enumerate(filelist):
            ...
            max_im_shrink = (0x7fffffff / 200.0 / (
                        image.shape[0] * image.shape[1])) ** 0.5  # the max size of input image for caffe
            ...
            [det2, det3] = multi_scale_test(image,
                                            max_im_shrink)  # min(2,1400/min(image.shape[0],image.shape[1])))  #multi-scale test
