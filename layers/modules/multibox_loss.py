...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        priors = priors[:loc_data.size(1), :]   # (H * W, 4) batch_size = 1情况就是全部的prior box
        num_priors = (priors.size(0))   # 当前的prior box 数量
        num_classes = self.num_classes

        # match priors (default boxes) and ground truth boxes

        loc_t = torch.Tensor(num, num_priors, 4)    # （batch_size, num_priors, 4) 表示每个prior box的位置误差(点*1，边*2）
        conf_t = torch.LongTensor(num, num_priors)  # （batch_size, numpriors) 表示该prior box的物体是否为人脸的误差
